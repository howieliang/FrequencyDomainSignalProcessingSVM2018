...
// e4_LinearSVM_FFT_Arduino_ThreeSensors_Window
...
void setup() {
  ...
  fftMerged = new ezFFT(bufferSize, sampleRate);
...
void draw() {
  ...
  float[] mergedSpectrum = new float[fft[0].getSpecSize()];
  float max = 0;
  for (int j = 0; j < fft[0].getSpecSize(); j++) {
    max = 0;
    for (int i = 0; i < sensorNum; i++) {
      float x = fft[i].getSpectrum()[j];
      if (x>max) max = x;
    } 
    mergedSpectrum[j] = max;
  }
  ...
  for (int j = 0; j < d; j++) {
    float x = fftMerged.getSpectrum()[j+LOW_THLD];
    if (x>energyMax) energyMax = x;
    appendArrayTail(FFTHist[j], x);
    //tempX[j] = x;
    if (b_sampling == true) {
      if (x>tempX[j]) tempX[j] = x;
    }
  }
  ...
  if (b_sampling == true) {
    ...
    if (sampleCnt == windowSize) {
      ...
      for (int n = 0; n<windowSize; n++) {
        appendArrayTail(modeArray, lastPredY); //append the prediction results to modeArray for visualization
      }
      b_sampling = false;
    }
  } else {
  ...
  fill(0);
  textSize(18);
  text("Threshold: "+energyThld, width-190, height-44);
  if (svmTrained) text("Last Prediction: "+lastPredY, width-190, height-20);
  else text("Current Label: "+type, width-190, height-20);
  float fps = frameRate;
