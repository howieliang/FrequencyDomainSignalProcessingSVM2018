...
void draw() {
  ...
  lineGraph(energyHist, 0., height, 0., height-150, 500., 150, 0, color(0,255,0));
  ...
  for (int i = 0; i < sensorNum; i++) {
    lineGraph(sensorHist[i], 0, height, 0, height-150, 500, 75, 0, color(255, 0, 0));
    lineGraph(diffArray[i], 0, 100, 0, height-75, 500, 75, 0, color(0, 0, 255)); //history of diff
  }
  ...
  pushMatrix();
  translate(550, 50);
  fft[0].drawSpectrogram(fftScale, 1024, false);
  popMatrix();
